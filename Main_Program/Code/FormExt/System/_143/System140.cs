...
{
    public class System143 : SwBaseForm
    {
        private Matrix aMatrix;
        private DBDataSource OPDN, PDN1;
        ...
        {
            OPDN = MyForm.DataSources.DBDataSources.Item("OPDN");
            PDN1 = MyForm.DataSources.DBDataSources.Item("PDN1");
        ...
        public override void FormDataLoad(ref BusinessObjectInfo businessobjectinfo, ref bool bubbleevent)
        {
            ...
            {
                var sum = 0.0;
                for (int i = 0; i < PDN1.Size; i++)
                {
                    var value = PDN1.GetValue("Quantity", i);
                    if (!string.IsNullOrEmpty(value))
                    {
                        sum += double.Parse(value);
                    }
